...
def test_whisper_factory_faster_whisper(factory):
    with patch("munajjam.transcription.whisper.WhisperTranscriber.__init__", return_value=None) as mock_init:
        transcriber = factory.create_whisper(WhisperBackend.FASTERWHISPER, "base", "cpu")
...
def test_whisper_factory_openai(factory):
    with patch("munajjam.transcription.whisper.WhisperTranscriber.__init__", return_value=None) as mock_init:
        transcriber = factory.create_whisper(WhisperBackend.OPENAI, "openai/whisper-large-v3", "cuda")
        assert isinstance(transcriber, WhisperTranscriber)
        mock_init.assert_called_once_with(
...
def test_whisper_factory_whisperx(factory):
    with patch("munajjam.transcription.whisperx.Whisperx.__init__", return_value=None) as mock_init:
        transcriber = factory.create_whisper(WhisperBackend.WHISPERX, "base", "cuda")
        assert isinstance(transcriber, Whisperx)
        mock_init.assert_called_once_with(model_name="base", device="cuda", compute_type="float16")
...
def test_whisperx_transcribe(mock_whisperx_module):
    ...
    mock_model.transcribe.return_value = {"segments": [{"start": 0.0, "end": 1.5, "text": "hello"}]}
